...
# ╔═╡ fd168b2b-242e-4e80-953a-cec00ccd4daa
begin
	using Distributions
	using StatsPlots
	using LinearAlgebra
...
# ╔═╡ 97cbc792-ca6b-11ee-21a2-a9e81216fb65
md"
===================================================================================
...
# ╔═╡ bd54afc5-bcbd-40f9-8c11-17dca28e9448
md"
This is a personal *interpretation*, *reconstruction* and *serialization* of the official corresponding [Turing.jl-tutorial](https://turinglang.org/v0.30/tutorials/01-gaussian-mixture-model/) into steps of increasing complexity. This seems to be helpful for learners (at least for me) because the original script is highly optimized and constrained.
...
md"
---
##### 1. Generation of Bivariate Bicluster Test Data
"
...
let N  = 300
	...
	rangeXY = 12.5
	length  = 100
	xs = range(-rangeXY, rangeXY; length = length)
	ys = range(-rangeXY, rangeXY; length = length)
	#--------------------------------------------------------------------------------
	mixtureModel = MixtureModel([MvNormal(μ[:, kk], 3.5 .* I) for kk in 1:K], w)
	...
	#--------------------------------------------------------------------------------
	# plot of data points
	scatter(x[1, :], x[2, :], label="data point", title=L"Data 1 (Mixed Gaussian Model with Prior $\mu[1], \mu[2]$)")
	#--------------------------------------------------------------------------------
	# plot of generative centroids
	scatter!([(μ[1, 1], μ[2, 1]), (μ[1, 2], μ[2, 2])], color=:violetred4, markershape = :cross, label="generative centroid", markersize=8, markerstrokewidth = 3)
	#--------------------------------------------------------------------------------
	# plot of density contours
    contour!(xs, ys, (x, y) -> logpdf(mixtureModel, [x, y]), widen=false, fill=false)
...
let N  = 300
	...
	# Data Generation
	#--------------------------------------------------------------------------------
	#               stretched standard deviation ====\/
	mixtureModel0 = MixtureModel([MvNormal(μ[:, k], 3.5 .* I) for k in 1:K], w)
	#                                           ^== explicit column vectors  |
	#                           continuous cluster membership ===============^
	...
	# ===============================================================================
	# plot of data points
	scatter(x[1, :], x[2, :], title=L"Data 2 (Post. Latent Membership $k[i]$ and $σ2$)", label="data point", zcolor=assignments)
	#--------------------------------------------------------------------------------
	# plot of the generative centroids
	scatter!([(μ[1, 1], μ[2, 1]), (μ[1, 2], μ[2, 2])], color=:violetred4, markershape = :cross, label="generative centroid", markersize=8, markerstrokewidth = 3)
	#--------------------------------------------------------------------------------
end # let
...
begin
	...
	D   = 2
	w   = [ 0.6, 0.4]                                # unequal weight of clusters
	μ0  = [-4.5 +0.5;                                # unconstrained centroids in R^2
		   +3.5 -0.6]
	rangeXY = 12.5
	length  = 100
	xs = range(-rangeXY, rangeXY; length = length)
	...
	#--------------------------------------------------------------------------------
	#               stretched standard deviation ====\/
	mixtureModel0 = MixtureModel([MvNormal(μ0[:, k], 3.5 .* I) for k in 1:K], w)
	#                                            ^== explicit column vectors  |
	#                            continuous cluster membership ===============^
	x = rand(mixtureModel0, N)
	#--------------------------------------------------------------------------------
	scatter(x[1, :], x[2, :]; label="data point", title="Data 3 (Mixed Multivariate Gaussian Data)")
	#--------------------------------------------------------------------------------
	# plot of generative centroids
	scatter!([(μ0[1, 1], μ0[2, 1]), (μ0[1, 2], μ0[2, 2])], color=:violetred4, markershape = :cross, label="generative centroid", markersize=8, markerstrokewidth = 3)
	#--------------------------------------------------------------------------------
	# plot of density contours
    contour!(xs, ys, (x, y) -> logpdf(mixtureModel0, [x, y]), widen=false, fill=false)
	# plot of the data points
	#--------------------------------------------------------------------------------
end # begin

# ╔═╡ 39d56e33-5092-4ba7-a283-e74ac7ef56a2
x

# ╔═╡ 6d3fe7ee-d5e5-4110-86a5-9d4751a3afaf
begin
	# ===============================================================================
	# Mixture Model
	#--------------------------------------------------------------------------------
...
# ╔═╡ 01337a9d-57c8-486d-99af-ee42e909fb04
# group(chains, :k)
# group(chains, :μ)
#--------------------------------------------------------------------------------
plot(chains[["μ[1][1]", "μ[1][2]", "μ[2][1]", "μ[2][2]"]], colordim =  :parameter, legend=true)

# ╔═╡ b95680a8-c045-4b03-9e65-66ed13bedf5d
describe(chains[["μ[1][1]", "μ[1][2]", "μ[2][1]", "μ[2][2]"]])

# ╔═╡ b5a94a85-4d62-45a8-b6fc-e53ab882c9df
# group(chains, :w)

# ╔═╡ 0cffe1f5-491b-4664-b051-db5bc60fcf4f
plot(chains[["w[1]", "w[2]"]])

# ╔═╡ b052e912-012e-43ac-a9a0-493bca4137a1
describe(chains[["w[1]", "w[2]"]])

# ╔═╡ 592152f0-407a-46bc-ae93-f8fc964f2fdf
plot(chains[["σ2"]])

# ╔═╡ 7451e9c1-3300-4cae-8e40-8aecab8f08e0
describe(chains[["σ2"]])

# ╔═╡ f4c03d6c-c283-497b-ae3f-a09ad033b630
begin
	#--------------------------------------------------------------------------------
	assignments = [mean(chains, "k[$i]") for i in 1:N]
	#--------------------------------------------------------------------------------
	μMean = Array{Float64}(undef, (D, K))
	for kk in 1:K
		μMean[:,kk] = [mean(chains, "μ[$kk][$i]") for i in 1:D]
	end # for kk
	# ===============================================================================
	scatter(x[1, :], x[2, :]; label="data point", title=L"Data 3 (Post. Membership $k[i], w[kk]$ and $σ2$)", zcolor=assignments)
	#--------------------------------------------------------------------------------
	# plot of generative centroids
	scatter!([(μ0[1, 1], μ0[2, 1]), (μ0[1, 2], μ0[2, 2])], color=:violetred4, markershape = :cross, label="generative centroid", markersize=8, markerstrokewidth = 3)
	#--------------------------------------------------------------------------------
	# plot of the posterior cluster centroids
	scatter!([(μMean[1,1], μMean[2,1]), (μMean[1,2], μMean[2,2])], color=:violet, label="posterior cluster centroid", markersize=6)
	#--------------------------------------------------------------------------------
end # begin

# ╔═╡ 0c9b7028-b77f-46dc-b3b6-79b7d5bee5aa
md"
---
##### References

- **Bishop, C.M.**; *Pattern Recognition and Machine Learning*, Heidelberg: Springer, 2006 (8th printing, 2009)

- **Turing.jl-team**; [Unsupervised Learning using Bayesian Mixture Models](https://turinglang.org/v0.30/tutorials/01-gaussian-mixture-model/); last visit 2024/03/09

"

# ╔═╡ 0e7be074-1d2e-40a3-a398-69ac9dc4fa7e
md"
====================================================================================
...
manifest_format = "2.0"
project_hash = "54ae9577e98fe37257b536933a6d1149fd5ed6ce"
